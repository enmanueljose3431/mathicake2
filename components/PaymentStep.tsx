...
interface PaymentStepProps {
  ...
  onUpdatePayment: (_data: { paymentReference?: string; amountBs?: string; paymentProof?: string | null }) => void;
  onBack: () => void;
  onComplete: () => void;
}
...
const PaymentStep: React.FC<PaymentStepProps> = ({ 
  ...
  const isComplete = paymentStrategy === 'FULL_ON_DELIVERY' || (paymentReference.length >= 6 && amountBs.length > 0 && paymentProof);
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        img.onload = () => {
          ...
          onUpdatePayment({ paymentProof: compressed });
        };
      };
          ...
          <>
            ...
            <section className="bg-white p-6 rounded-[2.5rem] shadow-soft border border-rose-50 space-y-4">
               ...
               <div className="space-y-3">
                 ...
                 <div className="pt-2">
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-2 ml-1 tracking-widest">Comprobante de Pago (Foto)</label>
                    {paymentProof ? (
                      <div className="relative w-full aspect-video">
                        <img src={paymentProof} alt="Comprobante" className="w-full h-full object-cover rounded-2xl border-2 border-primary/20" />
                        <button 
                          onClick={() => onUpdatePayment({ paymentProof: null })}
                      ...
                      <label className="w-full flex items-center justify-center gap-3 bg-primary/5 border-2 border-dashed border-primary/20 rounded-2xl p-4 cursor-pointer hover:bg-primary/10 transition-all">
                        <span className="material-icons-round text-primary">add_a_photo</span>
                        <span className="text-[10px] font-black text-primary uppercase tracking-widest">Subir Capture</span>
