...
interface SummaryStepProps {
  appState: AppState;
  onUpdate: (_data: Partial<AppState>) => void;
  onBack: () => void;
  onConfirm: () => void;
  config: AppConfig;
  user: any;
...
const SummaryStep: React.FC<SummaryStepProps> = ({ appState, onUpdate, onBack, onConfirm, config, user }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-background-light animate-fadeIn overflow-hidden">
      ...
      <main className="flex-1 p-6 space-y-6 overflow-y-auto no-scrollbar pb-10">
        ...
        {!user && (
          <div className="bg-primary/5 border-2 border-primary/20 p-5 rounded-[2rem] flex items-start gap-4 animate-fadeIn">
            ...
            <div>
              <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-1">Inicio de Sesión Requerido</p>
              <p className="text-xs text-slate-600 font-medium leading-relaxed">
                Para finalizar tu pedido y que puedas hacerle seguimiento, necesitamos que inicies sesión con tu cuenta de Google.
              </p>
            </div>
          </div>
        )}
      ...
      <footer className="bg-surface-light p-5 md:p-8 z-50 shadow-[0_-15px_35px_rgba(0,0,0,0.03)] rounded-t-[2.5rem] md:rounded-t-[3.5rem] border-t border-gray-100 shrink-0">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-5 md:gap-8">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            >
              {user ? 'Confirmar Pedido' : 'Ingresar para Continuar'}
              <span className="material-icons-round text-base">{user ? 'check_circle' : 'login'}</span>
            </button>
          </div>
        </div>
      </footer>
